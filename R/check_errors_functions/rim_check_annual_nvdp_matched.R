<<<<<<< HEAD:R/check_errors_functions/rim_check_annual_nvdp_matched.R
...
rim_check_annual_nvdp_matched <- function(samples_imported) {
  tryCatch({
    ...
    # This errors must be used only in annual, when the Fishing Ground and DCF
    # Metier is filled:
    err$coherence_dcf_fishing_ground_rim_stratum_origin <- coherence_dcf_fishing_ground_rim_stratum_origin(
      catches
    )
    err$coherence_dcf_metier_rim_stratum_origin <- coherence_dcf_metier_rim_stratum_origin(
      catches
    )
    err$emptyFishingGround <- empty_values_in_variables(
      catches,
      "CALADERO_DCF",
      "catches"
    )
    err$emptyMetier <- empty_values_in_variables(
      catches,
      "METIER_DCF",
      "catches"
    )
    err$errors_cod_id <- validate_cod_id(muestreos_up$catches)
    ...
    #
    # ---- IN LENGTHS ----
    # err$all_categories_measured <- all_categories_measured(catches, lengths_sampled)
    #
    ...
    #
    # # comment in annual:
    # # err$with_historical_size_range <- species_without_historical_range(muestreos_up$lengths)
    #
    # # comment in annual:
    # # err$size_range <- lengths_outside_size_range(muestreos_up$lengths)
...
>>>>>>> origin/develop:rim_check_annual_nvdp_matched.R
